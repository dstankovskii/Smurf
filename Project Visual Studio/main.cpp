...
float mapX = 0, mapY = 0; // переменные смещения карты
...
public:
	...
	Sprite spr;
		GAMER (Texture &image)
		{
			...
			spr.setTextureRect (IntRect (12, 1, 39, 62)); // вырезка из спрайт-листа
			rect = FloatRect (40, 350, 39, 62); // начальные координаты героя
	...
	void up (float t)
	{
		...
		if (Air) { dy = dy + 0.01*(t/20); } // если в воздухе
		rect.top += dy*t;
		Air = true;
		side (1);

		Kdr += 0.01*t;
		if (Kdr > 4) {Kdr -= 4; Ldr += 1;} // количество кадров в одной строке
		if (Ldr > 3) {Ldr -= 4;}
...
int main ()
{
	...
	printf (" LEFT - движение влево\n");
	printf (" UP - прыжок\n");
	printf (" DOWN - ускорение\n");
	printf ("\n ВНИМАНИЕ!\n");
	printf (" для окончания игры необходимо собрать все листья\n");
	printf (" не задевайте смерть\n");
	printf ("\n для продолжения нажмите ENTER\n");
	getchar();
	
	// карта теперь в отдельном файле
	...
	bool fullbonus;
	// все эти дополнительные Rпеременные нужны для предотвращения зацикливания звуков!
	...
	while (window.isOpen ())
	{
		...
		{
			if (!smurf.Air) { smurf.dy =- 0.42; smurf.Air = true; foot.play(); } // высота прыжка
		}
		...
		// функция столкновения двух rect
		for (e = 0; e < 9; e++)
			if  ((bot[e].rect.intersects (smurf.rect)) && Rcry == false) 
			{ cry.play (); Rcry = true; smurf.rect.left = 40; mapX = 1; smurf.rect.top = 450; smurf.rect.left = 40; }

		// яма
		if ((smurf.rect.top) > 763 && Rcry == false) { cry.play (); Rcry = false; smurf.rect.top = 450; smurf.rect.left = 40; mapX = 1; }
		Rcry = false;
